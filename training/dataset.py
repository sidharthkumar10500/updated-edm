...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # assert image.dtype == np.uint8 # comment this to work with numpy floats
        if self._xflip[idx]:
    ...
    def resolution(self):
        assert len(self.image_shape) == 3 # CHW
        # assert self.image_shape[1] == self.image_shape[2] # sidharth: no longer needed
...
#----------------------------------------------------------------------------
# NOTE new dataset class used for MRI data, taken from Brett
...
class NumpyFolderDataset(Dataset):
    ...
    def _load_raw_image(self, raw_idx):
        ...
        with self._open_file(fname) as f:
            image = np.load(f)
            # image = np.load(f)[None,:,:]#adding an extra dimension for the absolute defined dataset and to be compatible with the format of the DATaset defined on top
        return image
