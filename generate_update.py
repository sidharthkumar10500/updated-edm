...
def ablation_sampler(
    ...
    GT = ksp_data['GT'].cuda()
    ##x_next = _ifft(kspace_undersampled)
    x_undersampled = (_ifft(kspace_undersampled)[None,None,...])
    x_undersampled_2channel = torch.view_as_real(x_undersampled).squeeze()[None,...]
    x_undersampled_2channel = torch.permute(x_undersampled_2channel, (0,3,1,2))
    scaling = torch.quantile(x_undersampled.abs(), 0.99)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])): # 0, ..., N-1
        ...
        # denoised_unscaled = x_next # unnormalize(denoised, norm_mins, norm_maxes) #we need to undo the scaling to [-1,1] first
        # denoised_unscaled = denoised / torch.linalg.norm(denoised, dim=(-1, -2), keepdims=True)
        # denoised_unscaled = denoised_unscaled * torch.linalg.norm(x_undersampled_2channel, dim=(-1, -2), keepdims=True)
        Ax = mask[None,None,...]*(_fft(torch.complex(denoised[0,0,...], denoised[0,1,...])) [None,None,...])
        if i>90:
            print(i, torch.linalg.norm(Ax, dim=(-1, -2), keepdims=True))
        DC_term = kspace_undersampled[None,None,...] - Ax
        external_grad = torch.sum(torch.linalg.norm(DC_term, dim=(-1, -2), keepdims=True))**2#torch.ones_like(x_cur)
        # denoised_unscaled.backward(gradient=external_grad)
        # network_grad = x_hat.grad
        network_grad = torch.autograd.grad(outputs=external_grad, inputs=x_cur)[0]
        meas_grad = network_grad
        meas_grad = meas_grad / torch.linalg.norm(DC_term, dim=(-1, -2), keepdims=True)
        likelihood_step_size = 1 # weighting to the likelihood grad
        x_next = x_next - (likelihood_step_size) * meas_grad


        # score_theta =  (1/ (sigma(t_cur)**2) )*(net(x_cur / s(t_cur), sigma(t_cur)) - x_cur )
        # x_input_forward_operator = (1/torch.sqrt(alpha_t_bar[i]))*(x_cur + (1 - alpha_t_bar[i])* score_theta)# DPS paper xo^
        # external_grad = torch.ones_like(x_cur)
        # x_input_forward_operator.backward(gradient=external_grad)
        # Ax_hat = mask[None,None,...]*_fft(x_input_forward_operator)
        # DC_term = kspace_undersampled[None,None,...] - Ax_hat # Data consistency term
        # # norm_DC_term = torch.linalg.norm(DC_term, dim=(-1, -2), keepdims=True)
        # # norm_DC_term.backward()# complex numbers creating issues with autograd at this step
        # meas_grad = x_cur.grad*2*(DC_term)
        # meas_grad = torch.abs(meas_grad / torch.linalg.norm(meas_grad, dim=(-1, -2), keepdims=True))
        # # meas_grad = _ifft(DC_term)
        # # meas_grad = torch.abs(meas_grad / torch.linalg.norm(meas_grad, dim=(-1, -2), keepdims=True))
        ...
        #     x_next = x_hat + h * (d_cur + 1*meas_grad ) # * (2/sigma_steps[i])
        # else
        ...
        if i%99==0:
            ...
            plt.figure(figsize=(12,10)); plt.imshow(np.abs(to_plot_data[0,...] + 1j*to_plot_data[1,...]),cmap='gray'); plt.tight_layout(); plt.savefig('Debug.png',dpi=100); plt.close()
...
@click.option('--outdir',                  help='Where to save the output images', metavar='DIR',                   type=str, required=True, default = 'out5')
